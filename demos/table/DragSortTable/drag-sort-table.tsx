...
const Demo = () => {
  ...
  const handleDragSortEnd1 = (_beforeIndex: number, _afterIndex: number, newDataSource: any) => {
    setDatasource1(newDataSource);
    message.success('修改列表排序成功');
  };
  const handleDragSortEnd2 = (_beforeIndex: number, _afterIndex: number, newDataSource: any) => {
    setDatasource2(newDataSource);
    message.success('修改列表排序成功');
  };
  const handleDragSortEnd3 = (_beforeIndex: number, _afterIndex: number, newDataSource: any) => {
    // 模拟将排序后数据发送到服务器的场景
